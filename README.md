...
Python은 Kakao나 PlayMCP 전송을 구현하지 않습니다. 향후 PM skill이 아래 `pending` 명령으로 재시도 가능한 Kakao 행을 원자적으로 claim합니다. 출력된 행은 SQLite에서 `dispatching`으로 바뀌고 attempt count가 증가하며, 매 attempt마다 예측할 수 없는 새 `claim_token`이 발급됩니다. 다른 동시 명령이나 재개 실행에는 다시 나오지 않습니다. PM skill은 pending JSON의 `claim_token`을 그대로 보존해 PlayMCP `MemoChat` 호출 뒤 `ack` 또는 `fail`에 전달해야 합니다. 이전 attempt token은 새 attempt를 완료하거나 실패시킬 수 없습니다. 공급자가 ID를 주지 않으면 provider ID로 정확한 문자열 `provider_acknowledged`를 사용합니다. 실패 설명에는 비밀값을 넣지 마세요.
...
uv run mybudongsan notify ack EVENT_ID --provider-id ID --claim-token TOKEN
uv run mybudongsan notify fail EVENT_ID --error "safe timeout" --claim-token TOKEN
```

`status`는 읽기 전용이며 claim이나 attempt count를 변경하지 않습니다. `pending`이 claim한 뒤 JSON을 출력하기 전 또는 출력한 뒤 프로세스가 종료되어도 `notify status RUN_ID --state dispatching`으로 현재 `EVENT_ID`, `claim_token`, attempt count와 안전한 payload를 다시 확인할 수 있습니다. 외부 공급자 전송 여부를 먼저 확인한 뒤, 전송되지 않았음이 확인된 경우에만 같은 token으로 `notify fail EVENT_ID --error "safe recovery" --claim-token TOKEN`을 실행해 해당 채널 행을 재시도 가능 상태로 돌리고 `pending`을 다시 호출합니다. 공급자가 받았는지 불명확할 때 자동 재시도하면 중복 메시지가 생길 수 있으므로 재시도하지 말고 수동으로 확인합니다. 이 수동 복구 계약은 외부 공급자와 SQLite 사이의 exactly-once 전달을 보장한다고 가장하지 않습니다.

완료·실패 Gmail도 먼저 `notify pending RUN_ID --channel gmail`로 같은 outbox claim을 얻은 뒤, 동일한 Keychain 자격 증명을 쓰는 `GmailNotifier` adapter로 plain-text UTF-8 메일을 전송하고 해당 claim token과 provider message ID로 `notify ack` 해야 합니다. Gmail 실패나 전송 전 crash도 `status` 조회와 동일 token의 명시적 `notify fail` 절차만 사용합니다. 자동화 테스트는 fake Gmail service만 사용하며 브라우저나 실제 Google API를 열지 않습니다.
