...
`research_bundle.json`은 네트워크 URL 없이 15개 발견, 7개 검증, 3개 심층 조사 관측값을 포함합니다. 실행 결과의 `report_path`는 절대 경로로 출력됩니다.
...
`run report`는 출력 루트 아래 `YYYY/MM/<request_id>_<run_slug>/`에 변경 불가능한 스냅샷을 생성합니다.
...
사용자 오류는 한국어 메시지와 0이 아닌 종료 코드로 반환됩니다. 진단이 필요할 때만 전역 옵션 `--debug`을 앞에 붙여 traceback을 확인합니다.
